        ...
        // Check inputs
            ...
            // Last check of the capacity before flushing the reservation and items to the database
                ...
                foreach ($reservations as $existingReservation) {
                    if ($existingReservation->getStatus() === 'CONFIRMED' || $existingReservation->getStatus() === 'PENDING') {
                        $reservationItems = $existingReservation->getReservationItems();
...
//
    #[Route('/pending', name: 'app_reservation_pending', methods: ['GET'])]
    public function reservationPending(
            Request $request,
            ReservationRepository $reservationRepository
            ): Response
        {
            $id = $request->getSession()->remove('pending_reservation_id');
            if ($id === null) {
